...
use tiled::LayerType::{GroupLayer, ImageLayer, ObjectLayer, TileLayer};
...
pub fn process_loaded_tile_maps(
    ...
    object_layer_dwellers: Query<(&TiledObjectID, Entity)>,
) {
    ...
    for changed_map in changed_maps.iter() {
        for (_, map_handle, mut map) in query.iter_mut() {
            let mut map: Mut<bevy_ecs_tilemap::Map> = map;
                ...
                for layer in tiled_map.map.layers() {
                    match layer.layer_type() {
                        ...
                        ObjectLayer(obj_layer) => {
                            let ids: Vec<u32> = obj_layer.objects().map(|obj| obj.id()).collect();
                            for existing_obj in object_layer_dwellers.iter() {
                                // if ids.contains(&existing_obj.0.0) {
                                commands.entity(existing_obj.1).despawn_recursive();
                                // }
                            }
                        }
                        // ImageLayer(_) => todo!(),
                            ...
                            ObjectLayer(obj_layer) => {
                                for obj in obj_layer.objects() {
                                    ...
                                    match &obj.shape {
                                        ...
                                        // tiled::ObjectShape::Ellipse { width, height } => todo!(),
                                        // tiled::ObjectShape::Polyline { points } => todo!(),
                                        tiled::ObjectShape::Polygon { points } => {
                                            println!(r#"found polygon "{}""#, obj.name);
                                            ...
                                            let line = ShapePath::build_as(&poly);
                                            ...
                                            commands
                                                ...
                                                .insert(TiledObjectID(obj.id()))
                                                .insert(Transform {
                                                    translation: Vec3::new(obj.x, -obj.y, 1.0),
                                        ...
                                        // tiled::ObjectShape::Point(_, _) => todo!(),
                                        _ => {}
                                    }
                                }
                            }
                            ImageLayer(_) => println!("skipping unsupported image layer"),
                            GroupLayer(_) => println!("skipping unsupported group layer"),
                        }
                    }
                }
            }
